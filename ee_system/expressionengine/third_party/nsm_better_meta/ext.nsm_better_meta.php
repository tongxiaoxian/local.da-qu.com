...
{if canonical_url} <link rel="canonical_url" href="{canonical_url}" /> {/if}',
	...
	public	function settings_form()
	{
		...
		$vars = array(
			...
			'message' => false,
            'self' => $this,
            'fields_by_channel' => $this->getChannelFieldsGroupedByChannelId(),
		);
    ...
    private function getChannelFieldsGroupedByChannelId()
    {
        $fieldsByChannel = array();

        // get the channel fields and group them by channel id
        $getChannelFieldsSql = <<<SQL
SELECT
    f.field_id,
    f.group_id,
    f.field_name,
    f.field_label,
    f.field_type,
    GROUP_CONCAT(c.channel_id SEPARATOR ',') as channel_ids
FROM exp_channel_fields AS f
LEFT JOIN exp_channels AS c ON c.field_group = f.group_id
WHERE f.field_type IN ('text', 'textarea', 'rte', 'wygwam', 'nsm_tiny_mce')
GROUP BY f.field_id
SQL;

        $getChannelFields = ee()->db->query($getChannelFieldsSql);
        foreach ($getChannelFields->result_array() as $channelField) {
            $applicableChannels = explode(',', $channelField['channel_ids']);
            foreach ($applicableChannels as $fieldChannelId) {
                $fieldId = $channelField['field_id'];
                $fieldLabel = sprintf(
                    '%s (%s)',
                    $channelField['field_label'],
                    ucwords($channelField['field_type'])
                );
                $fieldsByChannel[$fieldChannelId][$fieldLabel] = $fieldId;
            }
        }

        return $fieldsByChannel;
    }
