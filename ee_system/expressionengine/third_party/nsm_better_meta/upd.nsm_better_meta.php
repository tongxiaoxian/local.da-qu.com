...
class Nsm_better_meta_upd
{
	...
	public function update($current = FALSE)
	{
        ...
        if (version_compare($current, '1.1.9', '<')) {
			$query = $EE->db->query('SELECT * FROM `exp_nsm_addon_settings` WHERE `addon_id` = ?', array($this->addon_id));
            foreach ($query->result_array() as $siteSettings) {
                $jsonSettings = json_decode($siteSettings['settings'], true);
                $newChannelSettings = array();
                foreach ($jsonSettings['channels'] as $channelId => $channelSettings) {
                    // add the new channel setting to the JSON object
                    $newChannelSettings[$channelId] = array_merge($channelSettings, array(
                        'fallback_description' => null,
                    ));
                }

                // merge the updated channel data back into the settings json
                $jsonSettings['channels'] = $newChannelSettings;

                // write the changes back to the database
                $EE->db->update(
                    'exp_nsm_addon_settings',
                    array('settings' => json_encode($jsonSettings)),
                    array(
                        'addon_id' => $this->addon_id,
