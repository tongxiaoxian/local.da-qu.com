...
class Nsm_better_meta
{
	...
	public function template()
	{
		 ...
		 */
        if (isset($this->EE->TMPL->layout_vars)) {
		...
		if (ctype_digit($entry_id) || $url_title) {
			...
			if ($query_result->num_rows > 0) {
                ...
                $channelSettings = $extension_settings['channels'][$entry_metadata['channel_id']];

                // get the fallback description if its been set
                if ('' === (string) $nsm_bm_metadata['description'] && $channelSettings['fallback_description'] > 0) {
                    $db = ee()->db;
                    $db->select('field_id_' . (int) $channelSettings['fallback_description'] . ' AS fallback_value');
                    $getFallbackDescription = $db->get_where(
                        'channel_data',
                        array('entry_id' => $entry_metadata['entry_id']),
                        1
                    );

                    foreach ($getFallbackDescription->result_array() as $fallbackDescriptionRow) {
                        $fallbackDescription = trim(strip_tags($fallbackDescriptionRow['fallback_value']));
                        $fallbackDescription = trim(str_replace(array("\n", "\t", '  '), array('. ', ' ', ' '), $fallbackDescription));
                        $fallbackDescription = (strlen($fallbackDescription) > 156) ? substr($fallbackDescription, 0, strpos($fallbackDescription, ' ', 156)) : $fallbackDescription;
                        $nsm_bm_metadata['description'] = $fallbackDescription;
                    }
                }
				
				$enabled_fields = $channelSettings['enabled_fields'];
				// Robots fields
